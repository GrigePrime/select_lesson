...
//     }
    ...
    function show_all_class(){
        global $smarty, $mysqli,$class_id,$op,$msg,$user_id;
        $op = 'addclass';
        ...
        if($class_id!=''){
            ...
            $result=$mysqli->query($sql) or die("在查詢資料庫時發生錯誤". $mysqli->error);
            if (mysqli_num_rows($result) != 0) {
                $sql = "SELECT `ccm_course` FROM `ccm` WHERE `ccm_id` = '{$user_id}'";
                $alreadyclass =$mysqli->query($sql) or die("在查詢資料庫時發生錯誤". $mysqli->error);
//                 echo "$alreadyclass";
                $i = 0;
